<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">

<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
		<title>SnapShop!</title>
		<meta charset="utf-8">
		<link rel="stylesheet" type="text/css" href="/css/jquery.fullPage.css" />
		<link rel="stylesheet" type="text/css" href="/css/main.css" />

		<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
	</head>
	
	
	<body>
		
		<div id="fullpage">
    <div class="section" id="section1">
	    <h1>SnapShop</h1>
	    <p>쇼핑&허세 SNS</p>
	    <p>당신의 물욕을 마음껏 표현하세요</p>
    </div>
    <div class="section" id="section2">
	    <h1>SnapShop for iOS</h1>
	    <p>iOS용 SnapShop 앱입니다.</p>
	    <a href="itms-services://?action=download-manifest&url=https://dl.dropboxusercontent.com/u/139545/snapshop.plist"><p>다운로드</p></a>
	    <a href="https://github.com/lsj333610dl/2014-03-NEXT-RealProject-iOS.git"><p>Github</p></a>
    </div>
    <div class="section" id="section3">
	    <h1>SnapShop for Android</h1>
	    <p>안드로이드용 SnapShop 앱입니다.</p>
	    <a href="/app/SnapShop.apk"><p>다운로드</p></a>
	    <a href="https://github.com/Woonohyo/SnapShop.git"><p>Github</p></a>
	    </div>
    <div class="section" id="section4">
	    <h1>SnapShop 서버</h1>
	    <p>SnapShop 서버입니다.</p>
	    <a href="https://github.com/comelf/L3Cache"><p>Github</p></a>
	    </div>
</div>
		
		<script type="text/javascript" src="/js/jquery.fullPage.js"></script>
	<script src="/js/main.js"></script>
	</body>
	
	
</html>	

